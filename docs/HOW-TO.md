...
An accuracy score card renders in the UI after completion. Fields at 95%+ show green, 85-94% is review-grade, and anything below 85% fails the production gate. Production readiness uses the exact verified production score when at least 5 exact-match checks are available, requires coverage of critical agents A1/A2/A4/A6/A7, and requires an evidence ledger with verifier checks, advisory Agent 17 sources, required per-agent field coverage, and field-level evidence for recommendation-critical numbers. Placeholder URLs, invalid or unreachable evidence URLs, stale source rows, unsupported claim values, unsupported estimates, missing industry-required real-data sources, and deterministic source-validation failures block production readiness.

If verifier confidence is below 95%, the pipeline expands failed agents to downstream dependent agents and runs up to three repair passes. Each repair pass injects exact failed fields, prior feedback, and a verified evidence pack, then records before/after failure counts before rerunning the verifier and production safety gate. Real-data source status includes source year, geography level, record count, and confidence. Fast-changing outputs such as real estate, competitors, grants, build-vs-buy, source audit, financial, and plan agents also use shorter cache TTLs, and all agent cache keys include the real-data fingerprint so old market evidence is less likely to survive into a new recommendation.
