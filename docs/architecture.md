...
If verifier confidence is below 95%, `_bhRunAccuracyRepairPass()` expands the failed agents to downstream dependents and runs up to three bounded repair passes with exact failed checks injected as repair context. Each pass records before/after failure counts, resolved failures, and new failures before rerunning the verifier and production safety gate. `_bhBuildEvidenceLedger()` records verifier checks, Agent 17 advisory declared sources, and recommendation-critical agent claims with field-level source metadata into `R.evidence_ledger`. Production readiness requires enough exact checks, critical-agent coverage across A1, A2, A4, A6, and A7, required field coverage for A3, A4, A5, A6, A10, A12, A16, and A17, valid non-placeholder evidence URLs, source-support checks for claim values, and field-level evidence for critical numbers such as rent, prices, revenue, costs, wages, counts, scores, percentages, capacity, timelines, fees, and funding amounts. `_bhValidateEvidenceUrls()` performs live reachability checks on evidence URLs before the final production gate. `_bhComputeProductionScorecard()` also derives an industry-profiled deterministic weighted evidence score across demand, competition, compliance, real estate, capital, and execution risk so the final model verdict cannot override contradictory evidence.

`R.real._source_status` tracks loaded, missing, and industry-required real-data sources, including per-source year, geography level, record count, source name, and confidence. Required-source gaps block production readiness. `R.real._fingerprint` is included in LLM cache identity so cached outputs are tied to the real-data bundle that produced them. Fast-changing outputs use shorter cache TTLs: real estate, grants, competitors, build-vs-buy, and source audit outputs expire after 45 minutes; competitive/financial/plan outputs expire after two hours; stable outputs keep the default TTL.
